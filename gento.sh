...
MAGENX_INSTALL_GITHUB_REPO="https://raw.githubusercontent.com/vlnd-net/magento2/main"
...
MAGENX_NGINX_GITHUB_REPO="https://raw.githubusercontent.com/vlnd-net/nginx-config/main/"
MAGENX_NGINX_GITHUB_REPO_API="https://api.github.com/repos/vlnd-net/nginx-config/contents"
...
# Config path
MAGENX_CONFIG_PATH="/opt/magento/config"
...
if [ -n "${DISTRO_INFO[0]}" ]; then
  ...
  if [ -f "/etc/os-release" ]; then
    ...
    if [ "${DISTRO_NAME%% *}" == "Ubuntu" ] && [[ "${DISTRO_VERSION}" =~ ^(24.04|22.04) ]]; then
      DISTRO_NAME="Ubuntu"
...
WHITETXT "-------------------------------------------------------------------------------------"
 ...
 if [ "$?" != 0 ]; then
  ...
  [ "${DISTRO_NAME}" == "Debian" ] && FULL_VERSION="$(cat /etc/debian_version )" || FULL_VERSION="$(lsb_release -d | awk '/(24|22)\.04.+/{print $3}')"
...
sudo rabbitmqctl wait /var/lib/rabbitmq/mnesia/rabbitmq_pid
sleep 5
...
for ENV_SELECTED in "${ENV[@]}"
  ...
  OWNER=$(${SQLITE3} "SELECT owner FROM magento WHERE env = '${ENV_SELECTED}';")
  sudo rabbitmqctl add_user rabbitmq_${OWNER} ${RABBITMQ_PASSWORD}
  sudo rabbitmqctl set_permissions -p / rabbitmq_${OWNER} ".*" ".*" ".*"
done
   else
    echo ""
...
for ENV_SELECTED in "${ENV[@]}"
 ...
 sudo useradd -d ${ROOT_PATH%/*} -s /bin/bash ${OWNER}
 mkdir -p ${ROOT_PATH}
 ## create magento php user
 sudo useradd -M -s /sbin/nologin -d ${ROOT_PATH%/*} ${PHP_USER}
 sudo usermod -g ${PHP_USER} ${OWNER}
 chmod 711 ${ROOT_PATH%/*}
...
curl -o /etc/nginx/fastcgi_params  ${MAGENX_NGINX_GITHUB_REPO}fastcgi_params
curl -o /etc/nginx/nginx.conf  ${MAGENX_NGINX_GITHUB_REPO}nginx.conf
